...
<title>商品列表</title>
	...
	<body >
		<div class="product">
			...
			<form action="productServlet.do" method="post"  name="frm">
			<div id="div-mid">
				<span><input type="button" value="添加" id="inp1" onclick="location.href=('addProductServlet.do')"/></span>
					...
					<table cellpadding="0px" cellspacing="0px">
							...
							<c:forEach items="${products.datas}" var="pmd" varStatus="status">
								<tr>
									...
									<td id="td6"><div><a href="updateProductServlet.do?getid=${pmd.id}">编辑<img src="img/xiu.png"/></a>&nbsp;&nbsp;&nbsp;<a href="productServlet.do?reqType=4&delet_id=${pmd.id}">删除<img src="img/del1.png"/></a></div></td>
								</tr>
							</c:forEach>
							</tbody>
					</table>
	...
	<a href="productServlet.do?reqType=2&pageNo=1&pageSize=5">首页</a>&nbsp;
	</c:if>
	
    <a href="productServlet.do?reqType=2&pageNo=${products.pageNo==1?products.pageNo:products.pageNo-1}&pageSize=5">上一页</a>&nbsp;
	
	<a href="productServlet.do?reqType=2&pageNo=${products.pageNo==products.totalPageSize?products.pageNo:products.pageNo+1}&pageSize=5">下一页</a>&nbsp;
	
	<a href="productServlet.do?reqType=2&pageNo=${products.totalPageSize}&pageSize=5">尾页</a>
	...
	第<select onchange="self.location.href=options[selectedIndex].value">
  		...
  		<c:when test="${products.pageNo==i+1 }">
  			<option value="productServlet.do?reqType=2&pageNo=${i+1} }&pageSize=5" selected="selected">${i+1}</option>
  		</c:when>
  		<c:otherwise>
  		<option value="productServlet.do?reqType=2&pageNo=${i+1}&pageSize=5" >${i+1}</option>
  		</c:otherwise>	
		</c:choose>
		</c:forEach>
			</select>页
